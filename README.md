...
## 注意
- 一人で閉じたネットワーク内で作業することを想定したものなので、セキュリティは一切考慮しておらず間違っても公開してはいけない
- そもそも HTTP のみ
- 同様におひとり様仕様なので、内部で管理しているキャッシュはスレッド**アンセーフ**である
- ワイドモニターを使っているのに、幅が固定で窮屈なので CSS で変更している。テンプレートファイルみたいに切り出した方が使いやすいかも？
